...
export const createTeam = async (previousState: RequestState | undefined, formData: FormData): Promise<RequestState> => {
...
export const editTeam = async (previousState: RequestState | undefined, formData: FormData): Promise<RequestState> => {
  const { id, name, flag, winner } = Object.fromEntries(formData);

  try {
    await connectDB();
    const team = await Team.findByIdAndUpdate(id, { name, flag, winner: winner ?? false });
    await team.save();
    revalidatePath("/admin/teams");
    return { success: true };
  } catch (error) {
